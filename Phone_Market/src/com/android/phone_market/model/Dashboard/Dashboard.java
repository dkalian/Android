package com.android.phone_market.model.Dashboard;
...
public class Dashboard extends ViewGroup {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		for (int i = 0; i < count; i++) {
			...
			this.mMaxChildHeight = Math.max(this.mMaxChildHeight,
					child.getMeasuredHeight());
		}

		// Measure again for each child to be exactly the same size.

		childWidthMeasureSpec = MeasureSpec.makeMeasureSpec(this.mMaxChildWidth,
				MeasureSpec.EXACTLY);
		childHeightMeasureSpec = MeasureSpec.makeMeasureSpec(this.mMaxChildHeight,
				MeasureSpec.EXACTLY);
		...
		setMeasuredDimension(resolveSize(this.mMaxChildWidth, widthMeasureSpec),
				resolveSize(this.mMaxChildHeight, heightMeasureSpec));
	}
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		while (true) {
			...
			hSpace = ((width - this.mMaxChildWidth * cols) / (cols + 1));
			vSpace = ((height - this.mMaxChildHeight * rows) / (rows + 1));
